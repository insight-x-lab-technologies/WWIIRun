...
- `GAME-01`: [`F1-01`](../specs/SPEC-F1-01-gameplay-scene-viewport-input.md) implementa a cena de gameplay, tick de integração e contrato comum de teclado/touch; o requisito permanece `Planned` até aeronave, movimento e vertical slice serem implementados e revisados.
- `UI-04`: além da evidência F0-07 abaixo, [`F1-01`](../specs/SPEC-F1-01-gameplay-scene-viewport-input.md) implementa viewport lógico portrait/landscape, safe areas, rotação durante a run e controles touch mínimos; o requisito permanece `Planned` até cobertura das telas futuras.
