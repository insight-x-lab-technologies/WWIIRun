...
- `GAME-02`/`ASSET-01`/`DET-01`/`PERF-01`: [`F1-04`](../specs/SPEC-F1-04-machine-gun-two-enemies.md) está `Done` após revisão independente: metralhadora, scout/interceptor, estado/hash/corpus v4, probe saturado e E2E/PWA foram verificados; os requisitos continuam `Planned` até a vertical slice completa.
- `GAME-02`/`ASSET-01`/`DET-01`/`PERF-01`: [`F1-05`](../specs/SPEC-F1-05-modular-destructible-structure.md) está `Done` após revisão independente: estrutura modular com pool de 16 slots, contatos por módulo e estado/hash v5; os requisitos continuam `Planned` até a vertical slice completa.
- `GAME-03`/`GAME-09`/`ASSET-01`/`DET-01`/`PERF-01`: [`F1-06`](../specs/SPEC-F1-06-coin-drops-collection-run-statistics.md) está `Done` após revisão independente: drop usa a stream `loot`, coleta e estatísticas transitórias entram no contrato v6, com corpus e gates rechecados; os requisitos continuam `Planned` até a vertical slice completa.
- `GAME-05`/`ASSET-01`/`DET-01`/`PERF-01`: [`F1-07`](../specs/SPEC-F1-07-parallax-replaceable-placeholders.md) está `Done` após revisão independente: quatro padrões técnicos repetíveis têm períodos próprios, offsets visuais por tick, reuso e teardown; o core e seus hashes permanecem isolados. `GAME-05` permanece `Planned` até a vertical slice completa.
- `GAME-08`/`UI-04`/`DET-01`/`PERF-01`: [`F1-08`](../specs/SPEC-F1-08-gameplay-hud.md) está `Done` após revisão independente: a projeção HUD read-only expõe vida, distância técnica, moedas transitórias, FPS local, nível provisório, velocidade e seed sem alterar o core; a correção visual confirma em Chromium que a camada contém somente seus quatro textos. Os requisitos permanecem `Planned` até a vertical slice completa.
- `GAME-09`/`UI-04`/`GAME-08`/`DET-01`/`PERF-01`: [`F1-09`](../specs/SPEC-F1-09-game-over-overlay-retry-home-summary.md) está `Done` após revisão independente: game over observa somente o snapshot terminal, congela modo/seed/distância/moedas/inimigos/duração/nível técnico, encerra no tick letal mesmo em frame acumulado e oferece retry/home sem alterar simulação. Os requisitos permanecem `Planned` até a vertical slice completa.
