...
- `COST-01`: [`F0-02`](../specs/SPEC-F0-02-project-scaffold.md) comprova que o scaffold e suas dependências diretas são gratuitos/open source; [`F0-03`](../specs/SPEC-F0-03-quality-toolchain-and-ci.md) estende a evidência à toolchain local e ao workflow CI sem secret, deploy ou runner premium obrigatório. O requisito permanece `Planned` porque se aplica ao lançamento completo e repositórios privados continuam sujeitos à franquia da conta GitHub.
...
- `PERF-01`: [`F0-06`](../specs/SPEC-F0-06-performance-harness-matrix-budgets.md) adiciona workload, schema, thresholds e matriz reproduzíveis. Conforme ADR-0007, os trios físicos de desktop Windows, iPhone 17 e Galaxy Tab S9 são avaliáveis com 119 janelas/595 s e preservam resultado `fail`; o coletor futuro mantém 120 janelas/600 s. O requisito permanece `Planned`: o workload é sintético e gameplay representativo ainda precisa ser validado.
- `ASSET-02`: [`F0-06`](../specs/SPEC-F0-06-performance-harness-matrix-budgets.md) limita bytes de raster/áudio emitidos. O requisito permanece `Planned`; dimensões, referências, licença e provenance pertencem a F0-08/F6-04.
