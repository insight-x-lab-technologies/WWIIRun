...
- 2026-07-05 — recheck independente da correção 1/2 sobre `49c239d..dc6b732`: `F1-02-UI-01`, `F1-02-LIFECYCLE-01`, `F1-02-PERF-01` e `F1-02-DIFF-01` confirmados como resolvidos, sem novo finding de implementação. Passaram 78/78 focados, 8/8 determinísticos, coverage 290/290, `npm run check` com 290 unitários/build/budget e `git diff --check` no range total. O reviewer não marcou `Done`: `npm run test:e2e` falhou antes dos testes porque o sandbox impediu iniciar o webserver, e a repetição externa foi recusada por limite de uso até 12:32; PWA não chegou a executar na cadeia. Lacuna classificada como `environment`, mantendo o item em `In review` até repetição independente de E2E produto+harness e PWA sem mudança de inputs.
- 2026-07-05 — retomada ambiental e aprovação final: `npm run test:e2e` passou 10/10 produto e 1/1 harness; `npm run test:pwa` falhou inicialmente apenas por `listen EPERM 127.0.0.1:4174` no sandbox e passou 10/10 na repetição externa autorizada. Nenhum input, runtime ou teste mudou entre as execuções. Os quatro findings permanecem resolvidos, todos os 16 critérios e gates aplicáveis possuem evidência, nenhum finding permanece; spec e roadmap movidos para `Done` e somente F1-03 liberado para `Ready`.
