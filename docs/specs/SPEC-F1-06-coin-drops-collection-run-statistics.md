...
- 2026-07-17 — revisão independente pós-correção aprovou F1-06 e moveu o item para `Done`. `F1-06-GATE-01`, `F1-06-PERF-01` e `F1-06-GATE-02` foram rechecados e fechados: coverage focada 83/83, coverage completa, `npm run check`, `npm run test:determinism` (12/12), `npm run test:e2e`, `npm run test:pwa` e `git diff --check ee18883..HEAD` passaram. Não houve findings remanescentes, de ambiente, de spec ou de review-probe.
