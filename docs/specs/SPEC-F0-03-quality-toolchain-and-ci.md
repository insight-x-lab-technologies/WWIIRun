...
### Evidências da correção F0-03-BOUNDARY-03/DOC-01 de 2026-07-01

- TDD reproduziu o bypass root-relative antes da correção: a matriz focada terminou com 19 falhas e 9 controles verdes, cobrindo as quatro camadas restritas, imports, reexports, `import()` e normalização de URL. Após a correção, a suíte final passou 32/32, incluindo raízes exatas com query Vite, segmentos percent-encoded, separadores equivalentes e controles permitidos em `/src/simulation` e `/src/shared`.
- O resolver semântico agora normaliza suffixes de URL, percent-encoding e separadores; specifiers Vite `/src` são resolvidos contra a raiz do projeto antes da mesma comparação canônica aplicada aos caminhos relativos. Phaser, imports dinâmicos computados e as proteções de globais anteriores permanecem inalterados.
- Uma fixture temporária real em `src/simulation` com import estático, reexport, import dinâmico root-relative e `self.Math.random()` fez `npm run lint` terminar com exit `1`, três diagnósticos `simulation-boundaries/no-external-imports` e um `no-restricted-globals`. A fixture foi removida e o lint seguinte passou com exit `0`.
- O README passou a refletir o `package.json`: documenta `test:determinism`, `content:validate`, `performance:budget`, o pipeline interno de `build`, o smoke adicional de performance e os limites ainda abertos, sem marcar `DET-01`, `PERF-01`, `PWA-01` ou `COST-01` como concluídos.
- Ambiente e gates frescos: Node `v24.15.0`, npm `11.12.1`; `npm ci` instalou 154 pacotes; `npm ls --depth=0` confirmou a árvore direta; cobertura passou 220/220; `npm run check` passou com 220 unitários, 7 determinísticos, typechecks, content validation, build e budget; `CI=1 npm run test:e2e` passou 6/6 produto e 1/1 harness; `git diff --check` passou. O warning conhecido do chunk Phaser permaneceu inalterado.
- O diff não altera dependências, lockfile, workflow, runtime do produto, arquivos de `src/simulation`, goldens ou baselines. Nenhuma publicação ou outra ação externa foi executada; a run pública `28559326341` continua sendo a evidência remota aceita para o critério 9.
...
- 2026-07-01 — quarta revisão independente: `Changes requested`. **High — F0-03-BOUNDARY-03:** `isRestrictedSimulationImport` retorna cedo para todo specifier que não começa por `.`, permitindo imports Vite root-relative como `/src/game/createGame` e `/src/services/save/index.ts`. Uma fixture real em `src/simulation` com esses dois imports, acompanhados de `@ts-expect-error` localizado, passou tanto `npm run lint` quanto `npm run typecheck` com exit `0`; portanto o terceiro critério continua falhando e a fronteira do ADR-0001 não está garantida. **Medium — F0-03-DOC-01:** o README não acompanha o gate efetivo: omite `test:determinism`, content validation e budget da ordem/scripts e ainda afirma que o harness não comprova determinismo ou budgets após F0-04/F0-06 estarem `Done`; o décimo critério foi reaberto. O finding remoto anterior foi resolvido: a API pública confirmou a run `28559326341` e o job `84673521702` como `success`, attempt 1, para `f40f716`, com instalação, gate local, Chromium e E2E verdes. Checks independentes: Node `v24.15.0`, npm `11.12.1`; `npm ci`, árvore direta, regressões ESLint 8/8, coverage 196/196, `npm run check` com 196 unitários/7 determinísticos/validator/build/budget, E2E 6/6 produto + 1/1 harness, provas negativas de formato/warning e `git diff --check` passaram. Todas as fixtures temporárias foram removidas; nenhum runtime, teste versionado, golden ou baseline foi alterado.
- 2026-07-01 — `F0-03-BOUNDARY-03` e `F0-03-DOC-01` corrigidos por TDD: resolução semântica cobre todas as camadas/formas root-relative e equivalências de URL, 32 regressões focadas e a fixture ESLint real passaram, e o README corresponde ao gate executável atual. Critérios 3 e 10 foram fechados; todos os gates locais aplicáveis passaram e o item retornou para `In review`, aguardando revisão independente.
- 2026-07-01 — quinta revisão independente: `Approved`, sem findings. O commit `3a30841` rejeita imports Vite `/src` para `app`, `game`, `platform` e `services` nas quatro formas cobertas, incluindo normalização de URL, e preserva controles para camadas puras; fixture real retornou quatro diagnósticos de fronteira e um de global proibido. O README corresponde ao grafo executável de scripts e preserva os limites dos requisitos amplos. Checks independentes: Node `v24.15.0`, npm `11.12.1`; `npm ci` (154 pacotes), árvore direta, regressões 32/32, coverage 220/220, `npm run check` (220 unitários, 7 determinísticos, validator, build e budget), E2E CI 6/6 produto + 1/1 harness e `git diff --check` passaram. A API pública reconfirmou run `28559326341` e job `84673521702` como `success`, attempt 1, para `f40f716`. Nenhuma fixture permaneceu; não houve alteração de runtime, dependência, workflow, simulation, golden ou baseline. Todos os dez critérios estão atendidos; item movido para `Done` e F0-07 liberado como `Ready`.
