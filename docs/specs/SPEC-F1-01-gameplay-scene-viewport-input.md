...
### Evidência das correções da segunda revisão em 2026-07-04

- `F1-01-PERF-02`: RED observou três criações de iterador por amostragem no hot path. `KeyboardInput` passou a manter eixos/contadores agregados nos eventos, e `PointerInput` passou a manter a máscara de ações; `sampleInto()` agora só lê números e atualiza armazenamento do chamador. O gate instrumenta `Set[Symbol.iterator]`/`Map.keys` e confirma zero chamadas durante a amostragem de produção, preservando teclas repetidas e duas teclas para a mesma ação.
- `F1-01-DOC-01`: `src/simulation/run/README.md` agora descreve que `stepRun` atualiza `state.input` in place.
- Focados: 71/71; `npm run check`: 274 unitários, 7 determinísticos, build, inspeção PWA e budget verdes. Coverage: 274/274, `simulation/run` 100% e input 92% branches.
- E2E: a primeira execução teve um timeout mobile isolado; reprodução focada passou 1/1 e repetição completa passou 10/10 produto + 1/1 harness. PWA passou 10/10 fora do sandbox.
- `git diff --check` passou. Goldens preservados nos SHA-256 `7d45e812fa33d866afd2d62d022d8e2d504fad6598d763d5c993ec200e7cd5da` e `46e3fa54c5ded1fd07e6a9bd9562ab16e923d552d843237b025a67586a5aab2b`; baselines físicos, dependências e workflows não mudaram.

### Evidência da correção da terceira revisão em 2026-07-04

- `F1-01-PERF-03`: o gate percorre o hot path de produção completo `CombinedInput → GameplaySession.update → stepRun` por 120 ticks após warm-up, mede as identidades retornadas e confirma armazenamento único, tick `121` e input canônico final. O RED mediu 120 objetos distintos; o GREEN mediu uma identidade.
- `GameplaySession.update()` agora possui um único resultado mutável pré-alocado e sobrescreve `ticks`/`dropped` antes de retorná-lo, inclusive no caminho inativo/pausado. A API e a semântica observável dos campos foram preservadas.
- Focados: 72/72. `npm run check`: 19 arquivos/275 unitários, 7 determinísticos, typecheck, build, inspeção PWA e budget verdes. Coverage: 275/275; `simulation/run` 100% e `GameplaySession` 100% de linhas.
- `npm run test:e2e`: 10/10 produto e 1/1 harness. `npm run test:pwa`: a tentativa no sandbox falhou antes dos testes com `listen EPERM 127.0.0.1:4174`; repetição autorizada fora do sandbox passou 10/10.
- `git diff --check` passou. Determinismo permaneceu verde e `tests/determinism`, baselines físicos, dependências e workflows não possuem diff no range F1-01.
...
- 2026-07-04 — segunda revisão independente: `Changes requested`. `F1-01-INPUT-01` e `F1-01-VIEWPORT-01` foram fechados: ownership/liberação de touch independente do teclado e redesenho das zonas passaram em testes unitários e Chromium. Finding `F1-01-PERF-02` (`Medium`): a evidência de `F1-01-PERF-01` demonstra somente identidade estável de `InputFrame`/`RunState.input`, não o contrato transversal de zero alocação obrigatória por tick após warm-up; `KeyboardInput.sampleInto()` e `PointerInput.sampleInto()` ainda percorrem `Set`/`Map` por iteradores, e nenhum teste/medição observa alocações do hot path. Finding `F1-01-DOC-01` (`Low`): `src/simulation/run/README.md` ainda afirma que `stepRun` copia o frame, embora a correção agora reutilize e atualize `state.input` in place. Gates frescos: `npm run check` passou com 273 unitários/7 determinísticos/build; coverage passou 273/273 com `simulation/run` 100%; E2E passou 10/10 + harness 1/1; PWA passou 10/10 fora do sandbox após `listen EPERM` local; `git diff --check`, autoria/committer, isolamento dos commits, hashes dos goldens e ausência de mudanças em baselines/dependências/workflows foram confirmados. Nenhum runtime/teste foi alterado pelo revisor.
- 2026-07-04 — findings `F1-01-PERF-02` e `F1-01-DOC-01` corrigidos por TDD; gate executável confirma ausência das fontes obrigatórias de alocação identificadas no tick, documentação e gates foram atualizados e o item retornou para `In review`.
- 2026-07-04 — terceira revisão independente: `Changes requested`. `F1-01-DOC-01` foi fechado: `src/simulation/run/README.md` corresponde à atualização in-place de `state.input`. Finding `F1-01-PERF-03` (`Medium`): a correção elimina os três iteradores identificados em `sampleInto()`, mas o teste instrumenta somente `Set[Symbol.iterator]` e `Map.keys`; ele não mede alocações no hot path completo. `GameplaySession.update()` ainda retorna um novo objeto `{ ticks, dropped }` em todos os caminhos, inclusive após executar ticks, portanto a evidência não sustenta o contrato transversal de zero alocação obrigatória por tick após warm-up. Gates frescos: focados 77/77; `npm run check` verde com 274 unitários, 7 determinísticos e build/budget; coverage 274/274 com `simulation/run` 100%; E2E 10/10 produto + 1/1 harness; PWA 10/10 fora do sandbox após `listen EPERM` local; `git diff --check` verde; determinismo, dependências, workflows e baselines físicos sem diff no range F1-01. Nenhum runtime/teste foi alterado pelo revisor.
- 2026-07-04 — `F1-01-PERF-03` corrigido por TDD: gate pós-warm-up cobre o tick de produção completo, o resultado de `update()` passou a reutilizar armazenamento e o item retornou para `In review` sem mudança em regras ou goldens.
- 2026-07-04 — quarta revisão independente: `Changes requested`. `F1-01-PERF-03` foi fechado funcionalmente: o caminho `CombinedInput → GameplaySession.update → stepRun` reutiliza frames, estado e resultado, e o gate pós-warm-up passou por 120 ticks. Finding `F1-01-TRACE-01` (`Medium`): a correção final permanece apenas no worktree sobre `882bb17`; `src/app/GameplaySession.ts`, o teste de regressão e a evidência documental não pertencem a um commit isolado, contrariando a evidência obrigatória de commit/diff com autoria e committer Codex e impedindo rollback/revisão reproduzível da unidade final. Gates frescos: focados 79/79; `npm run check` verde com 275 unitários, 7 determinísticos, typecheck, build, inspeção PWA e budget; coverage 275/275 com `simulation/run` 100% e `GameplaySession` 100% de linhas; E2E 10/10 produto + 1/1 harness fora do sandbox após falha de abertura do web server no sandbox; PWA 10/10 fora do sandbox; `git diff --check` verde; goldens, baselines físicos, dependências e workflows sem diff. Nenhum runtime/teste foi alterado pelo revisor.
- 2026-07-04 — `F1-01-TRACE-01` corrigido: a unidade final foi validada e materializada como sucessor direto de `882bb17`, com autor e committer `Codex <codex@openai.com>`. Focados passaram 72/72; `npm run check` passou com 275 unitários, 7 determinísticos, typecheck, build, inspeção PWA e budget; coverage passou 275/275 com `simulation/run` 100% e `GameplaySession` 100% de linhas; E2E passou 10/10 produto + 1/1 harness; PWA passou 10/10 fora do sandbox após `listen EPERM` local; item retornado para `In review`.
- 2026-07-04 — quinta revisão independente: `Approved`, sem findings. `F1-01-TRACE-01` foi fechado: `a5b2ee4` é sucessor direto de `882bb17`, contém runtime, regressões e evidência documental da correção final, e possui autor/committer `Codex <codex@openai.com>`. Os 16 critérios foram revalidados; focados passaram 79/79, `npm run check` passou com 275 unitários/7 determinísticos/build/budget, coverage passou 275/275 com `simulation/run` 100% e `GameplaySession` 100% de linhas, E2E passou 10/10 + harness 1/1 e PWA passou 10/10 fora do sandbox após `listen EPERM` local. Determinismo, baselines, dependências e workflows permaneceram sem diff. Item movido para `Done` e `F1-02` liberado como `Ready`.
