...
- 2026-07-17 — recheck independente da correção 1/2 retornou `Changes requested`:
  - `F1-05-REGRESSION-01`, `F1-05-DET-01` e `F1-05-TRACE-01` foram confirmados resolvidos: schema/cursor v5 e o discriminante negativo passam, `npm run test:determinism` passou 10/10 em 1,28 s e `git diff --check 0bf1585..HEAD` passou.
  - ID: `F1-05-PERF-01`; Severidade: High; Critério: AC-07 e AC-11; Evidência: `npx vitest run tests/unit/run.test.ts --reporter=verbose` falhou em `tests/unit/run.test.ts:299` por exceder o timeout de 5.000 ms (6.629 ms observados) ao comparar profundamente quatro `RunState`, incluindo scratch prealocado fora do hash; o focado agregado também falhou 1/73 por este mesmo timeout; Impacto: o gate unitário e, por consequência, `npm run check` não concluem no timeout configurado, e a prova de equivalência ainda mede buffers diagnósticos não canônicos; Owner: implementation; Correção: comparar apenas o estado canônico ou hashes canônicos para frame/batch/partition, preservando uma prova separada das identidades/caps do scratch sem igualdade profunda de buffers completos; Recheck commands: `npm run test:unit -- tests/unit/run.test.ts tests/unit/structures.test.ts tests/unit/broadPhase.test.ts tests/unit/combat.test.ts && npm run test:unit:coverage && npm run check`.
  - Lacuna de ambiente (não é finding de implementação): PWA permanece não verificável neste sandbox pelo mesmo `listen EPERM 127.0.0.1:4174`; não foi repetida sem mudança de ambiente.
- 2026-07-17 — correção de implementação 2/2 concluída e retornada para `In review`: `F1-05-PERF-01` troca a igualdade profunda de `RunState` por equivalência de hash canônico em frame/batch/partition e mantém assert separado para identidade dos buffers scratch. Os focados, coverage, determinismo, `npm run check` e `git diff --check 0bf1585..HEAD` passaram; o timeout de 5 s não foi alterado. A lacuna PWA continua ambiental e não foi tocada.
- 2026-07-17 — recheck final independente aprovado sem findings de implementação: `F1-05-PERF-01` foi resolvido por hashes canônicos para frame/batch/partition e identidades separadas de `broadPhase`, `candidateCodes` e `contactCodes`. `tests/unit/run.test.ts` passou 62/62 em 5,83 s, determinismo passou 10/10 em 2,11 s, coverage focal, `npm run check` e `git diff --check 0bf1585..HEAD` passaram. A PWA permanece uma lacuna ambiental conhecida (`listen EPERM 127.0.0.1:4174`), sem mudança de ambiente que justificasse repetição; não há finding atribuível à implementação. F1-05 foi movido para `Done` somente por esta revisão independente; nenhum item vizinho foi alterado.
