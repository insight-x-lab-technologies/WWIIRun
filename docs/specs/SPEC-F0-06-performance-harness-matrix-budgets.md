...
A spec foi aprovada após o proprietário registrar os aparelhos físicos acessíveis: Lenovo ThinkPad T430u com Windows, iPhone 17, iPad 10ª geração, Samsung Galaxy Tab S9 e um desktop com Intel Core i5-9600KF e GPU informada como GeForce RTX 2600 SUPER, cujo modelo exato ainda deve ser confirmado antes da medição. O ThinkPad cumpre o papel inicial de desktop com GPU integrada e o iPhone cumpre o papel celular obrigatório; os tablets ampliam a matriz sem substituir esses papéis.
...
- A matriz alvo tem quatro papéis: desktop com GPU integrada, Android intermediário com pelo menos 4 GB, iPhone/Safari suportado e tablet Android ou iPad. Cada papel deve registrar modelo concreto; emulação de viewport não conta como aparelho real.
...
- [ ] Given aparelho ou métrica que falha, when a entrega é documentada, then o valor observado permanece registrado e vira finding/item corretivo; workload, threshold e expected não são afrouxados no mesmo diff para fabricar aprovação.
...
- [ ] A documentação registra como executar/exportar, matriz, commit, relatórios brutos agregados, resumo, limitações, findings e próximo passo exato.
...
- não executado/não alegado: três repetições físicas de 30 s + 600 s no ThinkPad e iPhone, inspeção manual de profiler quando necessária e publicação dos seis JSONs/resumo. Nenhum baseline fictício foi criado.
