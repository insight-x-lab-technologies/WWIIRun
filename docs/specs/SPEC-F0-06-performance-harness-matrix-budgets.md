...
Dependências: `F0-02` (`Done`), ADR-0001 (`Accepted`), ADR-0006 (`Accepted`)
...
- [ ] Given uma medição completa, when warm-up e 600 segundos terminam, then o relatório v1 contém todos os campos, percentis nearest-rank, janelas de cinco segundos, capacidades e cinco ciclos/quatro recriações.
...
- [ ] Given três repetições válidas em um aparelho, when thresholds são avaliados, then cada repetição e o aparelho recebem pass/fail reproduzível, sem arredondamento que esconda estouro.
...
- [ ] Given asset eager/lazy, duplicado ou remoto nas fixtures, when o grafo é medido, then bytes são classificados/contados uma vez e URL remota é rejeitada.
- [ ] Given matriz publicada, when a evidência é revisada, then há modelos/ambiente concretos e três relatórios válidos para ao menos um desktop e um celular físicos no mesmo commit/workload.
...
- intake físico histórico: iPhone 02–04 e Galaxy Tab S9 02–04 formam conjuntos distintos e ambientalmente comparáveis no commit `1d75de79e7f5f340787a88e7d018a3a406bf59c0`, mas cada JSON tem 119 janelas. O avaliador corrigido retorna `not-evaluated`; failures, valores e hashes anteriores permanecem no resumo de intake.
...
- validação histórica dos trios: antes da correção, o teste focado passou 12/12 e a avaliação direta retornou `fail` para iPhone e tablet. Depois da correção de cobertura, os três conjuntos retornam `not-evaluated` com `run-N-incomplete-fps-windows`; hashes e JSONs não foram alterados.
...
- trio desktop histórico: `docs/performance/baselines/2026-06-29/desktop-primary-run-01..03.json`, hashes únicos `1ad31f10…`, `7048dd3e…` e `2e3ee063…`; cada run preserva o `fail` original por frame p95/Long Tasks, mas a avaliação agregada corrigida é `not-evaluated` por cobertura de 595/600 s.
- conjunto obrigatório pendente: desktop Windows runs 01–03 e iPhone runs 02–04 históricos são distintos e ambientalmente comparáveis no commit `1d75de79e7f5f340787a88e7d018a3a406bf59c0`, mas não satisfazem a cobertura de 120 janelas. Nova coleta é necessária para ambos.
...
- lifecycle atual: F0-06 está `In progress`; critérios de medição completa, avaliação de três repetições e matriz obrigatória permanecem abertos até três novas runs de desktop + três de iPhone com 120 janelas. `PERF-01`, `UI-04` e `ASSET-02` continuam `Planned`.
